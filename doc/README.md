...
| 3 | [contract/schemas/](contract/schemas/) | Machine-readable JSON Schemas for config, point libraries, and sample, command, status and device-manifest messages. |
...
| 18 | [rfc/0006-contract-0.2.md](rfc/0006-contract-0.2.md) | **Proposed, for review (revision 2).** Contract 0.2: `mode` folded into `datatype`, a slim sample envelope, a retained per-device *manifest*, writes in engineering units (a defect fix `range` depends on), typed signal metadata with a driver-enforced `range` and a runtime-applied `publish` policy, the runtime answering thin-edge commands directly, `describe` → `manifest` + a separate Cumulocity renderer, an optional (recommended withdrawn) removal of the protocol topic level, and a C freeze at 0.1 while it lands — each with today/proposed examples and a feedback line, closing with the list of what 0.2 knowingly gives up. Prompted by a comparison with the Model Hardware Standard. |
