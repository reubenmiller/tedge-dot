...
| 18 | [rfc/0006-contract-0.2.md](rfc/0006-contract-0.2.md) | **Proposed, for review.** Contract 0.2: `mode` folded into `datatype`, a slim sample envelope, a retained per-device *manifest*, typed signal metadata with a driver-enforced `range`, no protocol level in topics, the runtime answering thin-edge commands directly, `describe` → `manifest` + a separate Cumulocity renderer, and a C freeze while it lands — each with today/proposed examples and a feedback line, prompted by a comparison with the Model Hardware Standard. |
