...
| **Sample publishing** | Serialize `Sample` to the envelope and publish to `sample/<point>` (non-retained). The envelope is a time series row (contract §5) — identity, value, quality — so nothing static per point is echoed: `raw` and `addr` are added only under `[connector] sample_debug`, and the unit, labels, `access`, `meta` and device `type` are on the manifest. |
| **Command routing** | Subscribe to the THIN-EDGE command topics (`te/device/+///cmd/+/+` and the service one) and drive them directly (contract §6): resolve the command type to a verb through `command_aliases`, act only on commands whose device AND points the live configuration has (§6.5 — other instances receive them too), validate against the command schema, run the state machine (`init→executing→successful|failed`), call `execute`, and publish results on the topic the request arrived on. Unknown fields (`c8y-mapper`, `origin`) are carried into every transition. A management verb on a device topic is claimed by the payload's `service` (§6.6). |
| **Command capabilities** | Publish the retained markers thin-edge expects (`te/device/<device>///cmd/<type>` = `{}`) for every command type a device answers, alongside its manifest, and clear them with it. |
| **Write units** | A write request's `value` is in engineering units (contract §4.2). The runtime inverts the point's linear transform before `execute`, rounds to nearest for an integer datatype, fails a value the datatype cannot hold, and echoes the engineering value in the result — so a module still receives, and still returns, the wire value. |
| **Write limits** | A point's `range` (contract §5.3) is checked on the engineering value before the transform is inverted and before `execute`; in a `write-batch` every entry is checked before the first write, so an out-of-range value fails the batch with nothing applied. A module never sees an out-of-range write. |
| **Publish policy** | A point's `publish` table (contract §5.4) — `on_change`, `deadband`, `min_interval`, `debounce` — is applied by the runtime to the sample stream, so every consumer sees one stream with the policy already applied. `bad` samples are never gated; `seq` is stamped before the gate, so a suppressed reading leaves a visible gap. |
...
| **Device manifest** | Publish each device's retained manifest (contract §8.2) — type, descriptor, and every point with its datatype/access/unit/labels/`meta` and its `parameter` (the sets it belongs to and how it should be presented) resolved — before its link status, republish it on every configuration change, and clear it (with the link status) when the device is removed or switched off. The SDK resolves the parameter sets; it renders them for no cloud. A Cumulocity DTM rendering is `tedge-dot manifest --format c8y-dtm`, in the binary crate, over the manifest — so the SDK stays a cloud-agnostic driver. |
